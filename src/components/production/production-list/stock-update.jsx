...
const StockUpdate = () => {
  ...
  useEffect(() => {
    ...
    const response = async () => {
      try {
        /** stock update list variable to be to be used to update the table UI */
          ...
          .then((res) => {
            if (res.data.error) {
              ...
              const currentRowData = (subItem) => {
                ...
                const order_ref = subItem.order_ref;
                const direction = subItem.direction;
                const stocked_date = subItem.date_in;
                const stocked_time = subItem.time_in;
                const after_order = subItem.qty_after_update;
                const before_order = subItem.qty_before_update;
                const qty_updated = subItem.qty_update;
  ...
  const footerProp = {
    ...
    totalPageNumbers: stockUpdateList?.length,
    handleNextPagination: () =>
      handleNextPagination(
        stockUpdateList,
        currentPage,
